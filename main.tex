...
This rule-based heuristic was chosen over a learned classifier because the relationship between mirror checks and manoeuvres is well-defined by driving standards, making a simple threshold both interpretable and auditable. The 5-second window was selected based on the observation that a mirror check performed more than 5 seconds before a manoeuvre may no longer reflect the current traffic state (Figure \ref{fig:safe_lane_change}).
...
The web application allows users to upload their footage, which is analysed and the results presented on an interactive display with synchronised video playback and navigation to detected events.
...
For computationally intensive analyses, particularly those involving long videos or multiple concurrent users, the server's single GPU became a bottleneck. SLURM cluster integration was added via SSH tunnelling, allowing analysis and recalibration jobs to be submitted to the university's GPU cluster. The backend manages job submission, monitors SLURM job status, and retrieves results from the shared filesystem, abstracting the cluster interaction entirely from the user.\\
...
Authentication is handled via JSON Web Tokens (JWT) with a 7-day expiry, chosen over session-based authentication because the stateless nature of JWTs avoids the need for server-side session storage and simplifies the deployment of multiple backend instances. Tokens are generated by the FastAPI backend and verified on each protected request. Passwords are hashed using bcrypt, selected for its resistance to brute-force attacks through configurable work factors. An optional email verification flow was also implemented. This account system was a prerequisite for the per-user calibration and admin features described below, as both require associating data with specific users.
...
\subsubsection{Adaptive Video Quality}
During user testing on mobile devices, it was observed that loading the full-resolution output videos (1920$\times$1080) over the university's Wi-Fi caused significant buffering, particularly when both road and driver-facing streams needed to be loaded simultaneously. Since two concurrent video streams double the bandwidth requirement compared to a single-video application, adaptive quality selection was particularly important. To address this, the system generates multiple quality variants (720p and 480p) of both output videos after analysis completes, using FFmpeg to transcode into web-compatible H.264. Upscaling is avoided: variants are only generated when the source resolution exceeds the target. The frontend estimates the user's effective bandwidth and automatically selects an appropriate resolution, with the option to manually override.
...
\subsubsection{Multi-File Upload and Error Handling}
