...
% Insert screenshot of SmartEye landing and ZenRoad landing, with links to figures in the text.
...
% Can you highlight any evidence you've found for this? Like that other current DMS systems do have these purposes to? Research - Nauto has insurance analytics for fleets. Include a reference to this: https://www.nauto.com/industries/insurance
\begin{itemize}
...
% Mention that this was actually the method suggested by our external advisor from JLR.
...
% User testing was difficult to conduct for several reasons. Participants were limited to individuals known to the author, due to ethical and practical concerns around general recruitment. While there were many potential participants who owned cars and driver's licenses, the largest limitation was the lack of dash-cam ownership: out of 19 potential driving participants, only 5 owned dash-cams. Therefore, this portion of the evaluation may be biased due to personal connections and has limited statistical significance.
...
Beyond quantitative accuracy, the system was evaluated through user testing sessions where participants uploaded their own driving footage, reviewed the results, and reported any errors they identified through the built-in error reporting mechanism. These reports (false positives and false negatives) were reviewed by the author through the administrator dashboard and used both for accuracy measurement and for identifying systematic failure modes. Participants were also asked qualitative questions about the usefulness of the tool and whether they would recommend it to someone learning to drive.

% Add more here - how users were recruited (those known to the author, fully qualified drivers), how they should have conducted their driving - just regular driving, record with phone or 2-in-1 system, survey questions that were asked - https://forms.gle/AUpb8mX9cFf1XoFQ8 - where it was sent/response got from (NextDoor app, LinkedIn, sent to friends and family).
...
% This is mentioned in the admin section. Maybe refer back to that?
The web application includes an error reporting mechanism that allows users to flag false positive detections (events the system detected that did not occur) and false negative detections (real events the system missed). During the deployment, users submitted 13 error reports: 9 false positives and 4 false negatives.
...
% Have some sort of introduction here so that there is some text between two headings
...
% Add citations for Viofo and the JLR range
The system would benefit from integration with existing dual-channel dash-cam systems, such as those manufactured by Viofo, which combine road-facing and driver-facing cameras in a single unit and could streamline the capture and upload workflow. Similarly, vehicles with factory-fitted interior cameras (such as the latest JLR range) already possess the necessary hardware, though integration with proprietary in-vehicle systems presents its own challenges. Even with such integrations, the requirement for specialised recording hardware remains a significant barrier for a learner driver feedback tool compared to software-only solutions.
