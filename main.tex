...
\subsection{Lane Detection Evaluation}
Lane detection was evaluated separately, since lane visibility is a prerequisite for lane change detection rather than a directly labelled event. Coverage was measured as the proportion of frames in which the model detected at least one lane ($\geq$1) or at least two lanes ($\geq$2), since two visible lanes are required for a lane crossing to be identified. Stability was measured as the proportion of consecutive frame pairs where the detected lane count did not change, indicating consistent rather than flickering detections.
...
Over a four-week period (late February to late March 2026), six external users registered accounts and submitted driving footage through the web application. In total, 23 unique video pairs (road-facing and driver-facing) were uploaded and successfully analysed, representing approximately 148 minutes of driving footage across a range of conditions and locations. Several users re-analysed the same footage after creating or refining their gaze calibration profiles; after deduplication (retaining the most recent calibrated analysis per unique video pair), the 23 analyses produced 170 safety events. Of the 23 unique analyses, 13 (57\%) used a personalised gaze calibration, while the remaining 10 relied on the system's default classifier.
...
Of the 9 false positive reports, 7 concerned the same video submission (User F's footage) and were predominantly right turn detections. Manual review through the administrator dashboard confirmed that these false positives were caused by a winding road with successive right-hand curves, producing visual odometry signals indistinguishable from genuine turns. This is consistent with the curved-road false positive pattern identified in the author-labelled evaluation (Section 6.1).
...
The user deployment confirmed that the system generalises beyond the author's own footage and vehicle. Error reports submitted by external users surfaced the same failure modes identified in the controlled evaluation, rather than novel categories of failure, suggesting that the system's weaknesses are well-characterised and stem from fundamental limitations of the sensing approach (visual odometry ambiguity on curved roads, lane detection model coverage) rather than overfitting to the author's driving style or camera setup.
