...
This section reviews the key computer vision techniques used in the project, covering lane detection, temporal tracking, gaze estimation and motion estimation as the three core components of the proposed system.
...
[Insert images of tracking results with and without the masking]\\
...
Authentication is handled via JSON Web Tokens (JWT) with a 7-day expiry limit. This was chosen over session-based authentication because the stateless nature of JWTs avoids the need for server-side session storage and simplifies the deployment for the backend. Tokens are generated by the FastAPI backend and verified on each protected request. Additionally, email verification works by requiring admin-approval after account creation. This effectively works as a white-list for accounts, which allows the researcher to carefully authenticate each account, ensuring the prevention of bad actors attempting to use the system. It also means the admin can check each user's driver's license as per the requirements by the ethical approval board.
...
\begin{figure}[H]
    ...
    \caption{(a) Clip editor. (b) Point editor.}
...
[Insert images of blurred face examples]
