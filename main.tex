...
Project evaluation was split into two parts: quantitative (author-labelled ground truth) and qualitative (user testing). For the quantitative portion, the author manually labelled manoeuvre events across a subset of the collected dataset and compared these labels against the system's detections. Qualitatively, user testing sessions and a general interest questionnaire were conducted. User testing was difficult to conduct for several reasons. Participants were limited to individuals known to the author, due to ethical and practical concerns around general recruitment. While there were many potential participants who owned cars and driver's licenses, the largest limitation was the lack of dash-cam ownership: out of 19 potential driving participants, only 5 owned dash-cams. Therefore, this portion of the evaluation may be biased due to personal connections and has limited statistical significance.\\
...
Across the 104 labelled ground truth events, the system produced 103 detections. Greedy temporal matching yielded 84 true positives, 19 false positives and 20 false negatives, giving an overall precision of 0.82, recall of 0.81 and F1 score of 0.81. The mean temporal error between matched ground truth and detection timestamps was 1.3 seconds, with the majority of matches falling within 1.5 seconds of the labelled time.\\

Performance varied across manoeuvre types. Right lane changes were detected most reliably, with 23 of 25 correctly matched (92\%), and left lane changes achieved 76\% recall (19/25). The remaining false positives for lane changes (6 of the 19 total) were predominantly caused by the lane detection model misclassifying bus lanes or faded road markings as lane boundaries, leading the system to detect a lane crossing where none occurred. Left turns and right turns achieved 63\% (19/30) and 67\% (16/24) respectively. The lower recall for turns is partly attributable to the visual odometry pipeline's sensitivity to dynamic objects and lighting conditions, which can suppress the yaw signal during complex junctions. Of the 19 false positives, 12 were classified as right turns. Upon reviewing these cases through the administrator dashboard, it was observed that the majority occurred on roads that curve rightward, producing a yaw accumulation pattern indistinguishable from a genuine right turn as far as the visual odometry is concerned. Since the model relies solely on camera motion, it has no way to differentiate between a road that bends to the right and a deliberate right turn at a junction. Mitigating this would require incorporating additional environmental context, such as detecting give-way lines, junction markings, or changes in road topology that distinguish intersections from curved road segments (Figure \ref{fig:right_turn_misclass}).\\
...
The confusion matrix also reveals that type misclassification between categories is relatively rare: the system rarely confuses lane changes with turns or vice versa. The most common type of confusion was between left turns and right turns (4 left turns detected as right turns), which occurs when the visual odometry briefly accumulates yaw in the wrong direction before correcting.\\

These results can be compared to those of Zekany et al.\ \cite{zekany2022finding}, whose DTW-based manoeuvre classification approach was discussed in Section 2.4. As noted in the literature review, Zekany et al.\ is the only published work that attempts to detect and classify individual driving manoeuvres from dashboard camera video alone, making it the closest point of comparison for this project despite the differences in application domain (their system targets offline video indexing, while ours targets real-time driver feedback). Table \ref{tab:cm_comparison} presents the confusion matrices from both systems side by side for the four shared manoeuvre types, with diagonal (correct classification) percentages shown in bold where one system outperforms the other.
...
\hline
Left Turn & \textbf{89\%} & 0\% & 0\% & 8\% & 63\% & 13\% & 0\% & 0\% \\
Right Turn & 0\% & \textbf{80\%} & 7\% & 13\% & 4\% & 67\% & 0\% & 0\% \\
Left Lane Change & 1\% & 18\% & 7\% & 70\% & 0\% & 4\% & \textbf{76\%} & 0\% \\
Right Lane Change & 0\% & 0\% & 2\% & \textbf{98\%} & 0\% & 0\% & 4\% & 92\% \\
\hline
\end{tabular}
\caption{Confusion matrix comparison between Zekany et al.\ and our system for the four shared manoeuvre types. Each row shows the percentage of ground truth events detected as each type. Bold highlights the higher diagonal (correct classification) accuracy. Zekany et al.\ percentages exclude their additional categories (U-Turn, K-Turn, Highway, Deceleration). Our percentages exclude false negatives (missed events).}
\label{tab:cm_comparison}
\end{table}

While Zekany et al.\ achieve higher accuracy on turn detection (89\% and 80\% for left and right turns respectively, compared to our 63\% and 67\%), their lane change detection exhibits a severe systematic failure: only 7\% of left lane changes are correctly classified, with 70\% misclassified as right lane changes. Their system essentially classifies almost every lane change as a right lane change regardless of its actual direction, rendering the left/right distinction unreliable. By contrast, our system correctly distinguishes between left and right lane changes, achieving 76\% and 92\% respectively, with only a single instance of a right lane change being misclassified as a left lane change (4\%). The false positives that do occur in our lane change detection are caused by environmental factors (bus lanes, faded markings) rather than by a fundamental confusion between directions.\\
...
\textbf{Ground Truth} & LT & RT & LLC & RLC & LT & RT & LLC & RLC \\
\hline
Left Turn & 89\% & 0\% & 0\% & 8\% & \textbf{83\%} & 17\% & 0\% & 0\% \\
Right Turn & 0\% & 80\% & 7\% & 13\% & 0\% & \textbf{94\%} & 0\% & 6\% \\
Left Lane Change & 1\% & 18\% & 7\% & 70\% & 0\% & 0\% & \textbf{95\%} & 0\% \\
Right Lane Change & 0\% & 0\% & 2\% & 98\% & 0\% & 0\% & 4\% & \textbf{96\%} \\
\hline
\end{tabular}
\caption{Like-for-like confusion matrix comparison, with both systems evaluated only on matched (detected) events, excluding false positives and false negatives. Bold highlights the higher diagonal accuracy between the two systems. Zekany et al.\ percentages exclude their additional categories (U-Turn, K-Turn, Highway, Deceleration). Our row percentages are computed over the 84 true positive matches.}
...
The two systems also differ in evaluation framing. Zekany et al.\ report a metric called $F_\text{eliminated}$ ($1 - \frac{TP + FP}{N}$), representing the fraction of video discarded as uninteresting. This reflects their design goal of video indexing rather than event-level classification. Our system targets real-time driver feedback, so we evaluate using standard event-level precision (0.82), recall (0.81), and F1 (0.81). Despite this difference, a direct per-manoeuvre comparison is possible by taking the best (maximum F1) operating point for each of Zekany et al.'s individual classifiers from their Table III. Table \ref{tab:f1_comparison} presents this comparison.
...
The systems also differ qualitatively in their failure modes. Zekany et al.\ acknowledge that left and right lane changes ``alias frequently'' in their system, confirmed by 70\% of left lane changes being misclassified as right lane changes in their confusion matrix. Our system avoids this directional confusion entirely, because lane change direction is determined by explicit lane boundary crossing detection rather than trajectory shape matching. The only cross-type confusions in our matched-events matrix are between left and right turns (17\% and 6\% respectively), caused by brief yaw oscillations in the visual odometry. Both systems share a susceptibility to road curves producing false turn detections.\\

\subsection{Lane Detection Coverage}
Lane detection achieved an average coverage of 81.7\% for detecting at least one lane and 75.5\% for detecting at least two lanes (the minimum required for lane crossing detection). Frame-to-frame stability was 96.9\%, indicating that the model rarely produces flickering detections. Coverage was comparable between day (82.5\% / 76.7\%) and night (79.4\% / 71.8\%) conditions, demonstrating that the LaneATT model generalises reasonably well across lighting conditions. The slight reduction at night is expected, as lane markings are less visible under headlights alone.\\

\subsection{Safety Event Distribution}
