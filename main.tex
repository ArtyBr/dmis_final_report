...
% Explain greedy temporal matching. Citation needed?
...
% Place a table here showing all of the counts of detections, false postiives/negatives for each class

% Make the sentence explaining the VO pipeline's lower recall more layman-friendly

Performance varied across manoeuvre types. Right lane changes were detected most reliably, with 23 of 25 correctly matched (92\%). False positives for lane changes (6 of the 19 total) were predominantly caused by the lane detection model misclassifying bus lanes or faded road markings as lane boundaries, leading the system to detect a lane crossing where none occurred. Left turns and right turns achieved 63\% (19/30) and 67\% (16/24) respectively. The lower recall for turns is partly attributable to the visual odometry pipeline's sensitivity to dynamic objects and lighting conditions, which can suppress the yaw signal during complex junctions. Of the 19 false positives, 12 were classified as right turns. It was observed that the majority occurred on roads that curve rightward, producing a yaw accumulation pattern indistinguishable from a genuine right turn as far as the visual odometry is concerned. Since the model relies solely on camera motion, it has no way to differentiate between a road that bends to the right and a deliberate right turn at a junction. Mitigating this would require incorporating additional environmental context, such as detecting give-way lines, junction markings, or changes in road topology that distinguish intersections from curved road segments (Figure \ref{fig:right_turn_misclass}).\\
...
% Referring to the confusion matrix before it's been shown?
