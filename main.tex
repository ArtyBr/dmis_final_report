...
% ... TODO
% The first two sentences in the Introduction section are claims with no evidence.
...
% "Initially a premium feature" - in what tool? Where is the evidence? Or is the evidence citation 4? It's not clear to me.
...
% "State-of-the-art research on this subject focuses on directly training NNs, such as Transformers, with video data as input (rather than images)." It's strange to have no citation in a sentence like this. For transformers, you could cite the "Attention is All You Need" paper, if you are referring to the architecture in general.
...
% Should there be a citation for vSLAM?
...
% Section 3.1 - "In line with modern ML approaches" - what does this mean? It's vague.
...
% "PCA was applied to transform the angle data into a lower-dimensional space (Figure 10b), resulting in more well-defined groupings of gaze directions." - What is the benefit of this? Or is this what you're trying to explain in the following sentence? It's unclear.
...
% "When a manoeuvre is detected (either a lane crossing from the lane tracker or a turn event from the visual odometry module), the Safety Monitor queries the driver’s gaze history over the preceding 5 seconds. If the driver checked the corresponding mirror (left mirror for rightward manoeuvres, right mirror for leftward manoeuvres) within this window, the event is classified as safe; otherwise, it is classified as unsafe." - Why 5 seconds? You did try to explain this shortly afterwards, but 5 seconds seems a little arbitrary.
...
% What about if the users can't be bothered with this? Is it mandatory?
...
% You're being inconsistent in how you display resolutions ("1920x1080" vs "720p"). Please pick one and stick with it. I'd go with the former (WxH) rather than the latter, unless the "p" is important in how your system processes the video (which I don't think it is). Also, explain what FFmpeg is.
...
% This is interesting, but you haven't discussed it much. Can you consider expanding, and at the very least, showing the dashboard visually?
