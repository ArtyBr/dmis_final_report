...
The application will analyse synchronised dash-cam and cabin footage to detect driving mistakes. This section defines the functional and non-functional requirements for MVP and outlines optional extensions.
...
% Move this paragraph and subsequent images into the implementation section, probably the lane detection park

In order to establish if these models would generalise to British streets and consumer hardware (dash-cams), they were also qualitatively tested on our self-collected driving dataset (Figure \ref{fig:drive_examples}). Three representative models were compared: PolyLaneNet, Ultra-Fast Lane Detection (UFLD) \cite{qin2020ultra} and LaneATT. The LaneATT model was found to perform the most accurately and reliably. A quantitative benchmark was infeasible for the project, as this would require extensive manual labelling, taking time away from feature development. 
...
The general interest questionnaire received 17 responses (16 fully licensed drivers, 1 current learner). Respondents were shown demo videos of the system analysing our driving footage before answering. Table~\ref{tab:survey_results} summarises the key findings.
...
Returning to the research question posed in the Introduction, the results confirm that a multi-modal system combining road-facing and driver-facing video can reliably detect driving mistakes. The system achieved an F1 score of 0.81 across 104 labelled events and 92\% classification accuracy on matched detections, demonstrating that the approach is viable for identifying failures to check mirrors before manoeuvres. More broadly, the results demonstrate that this multi-modal approach can identify driving mistakes that neither data source could detect in isolation. The system's detection and classification performance compares favourably against the closest existing work by Zekany et al., particularly for lane changes where their system suffers from severe directional confusion that our explicit lane boundary crossing approach avoids entirely. This validates the core hypothesis of the project: that synchronising driver state with vehicle movement provides a richer and more accurate understanding of driving behaviour than analysing either stream alone.

The user deployment confirmed that the system generalises beyond the author's own footage and vehicle. Error reports submitted by external users showed the same failure modes identified in the controlled evaluation, rather than novel categories of failure, suggesting that the system's weaknesses are well-characterised and stem from fundamental limitations of the sensing approach (visual odometry ambiguity on curved roads, lane detection model coverage) rather than overfitting to the author's driving style or camera setup.

The evaluation also revealed that per-user gaze calibration is essential for reliable safety classification. The difference in accuracy between calibrated and uncalibrated analyses was substantial, confirming that a one-size-fits-all gaze model is insufficient given the variation in camera placement, seating position, and facial geometry across users. The iterative development of the calibration interface (from clip editor to point editor, reducing the process from five minutes to under one minute) illustrates a broader lesson: the usability of calibration and setup workflows is as important as the accuracy of the underlying models, since users who are frustrated by the process may disengage before obtaining useful results.
